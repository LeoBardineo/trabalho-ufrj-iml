...
class ModelPredictResult(BaseModel):
    ...
    full_tree: dict | None = None

class PredictRequest(BaseModel):
    url: str
    model_key: str | None = None  # Mantido opcional para retrocompatibilidade

class PredictResponse(BaseModel):
    url: str
    features: dict
    predictions: dict[str, ModelPredictResult]
...
def predict(body: PredictRequest):
    ...
    for model_key in MODELS.keys():
        try:
            ...
            predictions[model_key] = ModelPredictResult(
            ...
            print(f"Erro ao processar modelo {model_key}: {e}")
            predictions[model_key] = ModelPredictResult(
                prediction=None,
                label=None
            )
...
def list_models():
    ...
    return {
        ...
        "models": [
            {"key": "arvore_otimizado",  "label": "Árvore Otimizada"},
            {"key": "arvore_base",       "label": "Árvore Base"},
            ...
            {"key": "isolation_forest",  "label": "Isolation Forest (Anomalias)"},
        ]
    }
